        ...
        switch (sub) {
          ...
          default:
            return {
              kind: 'error',
              text: t(
                '用法: /spec init | new <目标> | status | list | show <id> | approve <id> | implement <id> | verify <id> [--deep] | archive <id> | validate [id] | edit <id>',
                'usage: /spec init | new <goal> | status | list | show <id> | approve <id> | implement <id> | verify <id> [--deep] | archive <id> | validate [id] | edit <id>',
              ),
            }
        }
      } catch (error) {
...
async function cmdStatus(ctx, agent, cwd, signal, t, lang) {
  ...
  if (!change) {
    if (!initialized) {
      return { kind: 'success', text: t('📐 openspec/ 未初始化 — 先运行 /spec init', '📐 openspec/ not initialized — run /spec init first') }
    }
    return { kind: 'success', text: t('📐 无活跃变更 — /spec new <目标> 创建一个', '📐 no active change — /spec new <goal> to create one') }
  }
