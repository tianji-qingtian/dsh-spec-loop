...
export function apply(ctx) {
  ...
  function SpecDock(props) {
    ...
    const idLabel = change.id
      ? change.id
      : (change.title && change.title !== '(untitled)' ? change.title : '…')
    const progress = Array.isArray(todos)
      ? { done: todos.filter((item) => item && item.status === 'completed').length, total: todos.length }
      : null
    ...
    return createElement('div', { className: 'spec-loop' },
      ...
      createElement('span', {
        className: 'spec-loop-status' + (busy ? ' spec-loop-status-active' : ''),
      }, t('status.' + change.status)),
      progress && (change.status === 'implementing' || change.status === 'implemented') ? createElement('span', {
  ...
  ctx.slots.inject('conversation.input.dock', () => ctx.slots.register(
    { name: 'conversation.input.dock', id: 'spec-loop', order: 0, label: 'Spec loop change card' },
    (props) => createElement(SpecDock, props),
  ))
}
