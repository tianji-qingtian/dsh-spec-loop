...
function messageText(message) {
	...
	return message.content.filter((b) => b && b.type === "text" && typeof b.text === "string").map((b) => b.text).join(" ");
}
/**
...
function validateDeltaText(text) {
	const lines = String(text || "").split(/\r?\n/);
	const sections = [];
	let current = null;
	let req = null;
	...
	for (const section of sections) {
		if (section.requirements.length === 0) errors.push(`${section.kind} section has no ### Requirement entries`);
		for (const r of section.requirements) if (r.scenarios === 0) errors.push(`requirement "${r.name}" has no #### Scenario`);
	}
	return {
		ok: errors.length === 0,
		errors,
...
* changed.
*/
function mergeDelta(deltaText, targetText) {
	const sections = parseDelta(deltaText);
	let out = targetText == null ? "" : targetText;
	let changed = false;
	for (const section of sections) for (const req of section.requirements) {
		...
		if (section.kind === "REMOVED") {
			...
			if (replaced.changed) out = replaced.text;
			else out = appendRequirementBlock(out, block);
			changed = true;
		} else {
...
function appendRequirementBlock(text, block) {
	return ensureRequirementsSection(text).trimEnd() + "\n\n" + block.trimEnd() + "\n";
}
function replaceRequirementBlock(text, name, block) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (m && m[1].trim() === name) {
			start = i;
			for (let j = i + 1; j < lines.length; j++) if (/^###\s/.test(lines[j])) {
				end = j;
				break;
			}
			break;
...
function transition(state, sub, arg, seq) {
	...
	if (sub === "new") return {
		...
		change: {
			id: "",
			title: arg.trim().slice(0, 200) || "(untitled)",
			status: "proposing",
			seq
		}
	};
	...
	if (sub === "approve" || sub === "implement" || sub === "verify" || sub === "archive") {
		const status = sub === "approve" ? "approved" : sub === "implement" ? "implementing" : sub === "verify" ? "verified" : "archived";
...
*/
function applySpecEvent(state, event) {
	...
	if (event.type === "command/done" && event.data) {
		...
		if (event.data.kind === "success") next = transition(next, entry.sub, entry.arg, Number(event.seq ?? 0));
	...
	if (event.type === "assistant/message") {
		...
		if (change.status === "implementing" && IMPLEMENTED_MARKER_RE.test(text)) return {
			...state,
			change: {
				...change,
				status: "implemented",
...
async function listDeltaFiles(ctx, cwd, changeId, signal) {
	...
	const deltas = [];
	for (const entry of entries) if (entry.type === "file" && entry.name.endsWith(".md")) deltas.push({
		cap: entry.name.replace(/\.md$/, ""),
		...
		for (const file of files) if (file.type === "file" && file.name.endsWith(".md")) deltas.push({
			cap: entry.name,
			rel: `${dirRel}/${entry.name}/${file.name}`
		});
	}
	return deltas;
}
async function validateChange(ctx, cwd, id, signal) {
	const errors = [];
	const warnings = [];
	if (!CHANGE_ID_RE.test(id)) return {
		ok: false,
		errors: [`change id "${id}" is not kebab-case ASCII`],
		warnings
	};
	...
	if (tasks === null) errors.push(`${dirRel}/tasks.md missing`);
	else if (countTasks(tasks).total === 0) errors.push(`${dirRel}/tasks.md has no - [ ] checklist items`);
	const deltas = await listDeltaFiles(ctx, cwd, id, signal);
	if (deltas.length === 0) errors.push(`${dirRel}/specs/ has no spec delta files`);
	else for (const delta of deltas) {
...
function newProposalPrompt(cwd, goal, answersText) {
	const zh = hasZh(goal);
	return [
		zh ? `你正在 ${cwd}/openspec/changes/<change-id>/ 下创建一份 OpenSpec 变更提案。` : `You are creating an OpenSpec change proposal in ${cwd}/openspec/changes/<change-id>/.`,
		zh ? "先读 openspec/project.md 与 openspec/specs/ 下的现有规格。产出：" : "Read openspec/project.md and openspec/specs/ first. Produce:",
		...
		"",
		zh ? "要求：change-id 用 kebab-case、动词开头（如 add-user-login）；不要开始实现；完成后不要勾选 tasks.md 的复选框。" : "Requirements: pick a kebab-case, verb-led change-id (e.g. add-user-login); do not start implementing; leave tasks.md checkboxes unticked.",
		zh ? "最后单独输出一行（这是机器标记，必须原样）：SPEC_CHANGE_ID: <change-id>" : "End your reply with a single line containing exactly (machine marker, verbatim): SPEC_CHANGE_ID: <change-id>"
	].join("\n");
}
...
async function collectWorkspaceFiles(ctx, cwd, signal) {
	...
	const MAX_FILES = 300;
	const MAX_FILE_BYTES = 102400;
	const MAX_TOTAL = 245760;
	const out = [];
	let total = 0;
...
function registerCommand(ctx) {
	ctx.commands.register({
		...
		input: { hint: "init | new <goal> | status | list | show <id> | approve <id> | implement <id> | verify <id> [--deep] | archive <id> | validate [id] | edit <id>" },
		handler: async (invocation) => {
			...
			if (!cwd) return {
				kind: "error",
				text: "no workspace directory for this session (session header has no cwd)"
			};
			...
			try {
				switch (sub) {
					...
					case "status": return await cmdStatus(ctx, agent, cwd, signal, t, lang);
					...
					default: return {
						kind: "error",
						text: t("用法: /spec init | new <目标> | status | list | show <id> | approve <id> | implement <id> | verify <id> [--deep] | archive <id> | validate [id] | edit <id>", "usage: /spec init | new <goal> | status | list | show <id> | approve <id> | implement <id> | verify <id> [--deep] | archive <id> | validate [id] | edit <id>")
					};
				}
			} catch (error) {
				...
				return {
					kind: "error",
					text: t(`/spec ${sub} 失败：${String(error)}`, `/spec ${sub} failed: ${String(error)}`)
				};
			}
		}
	});
...
async function cmdInit(ctx, cwd, signal, t) {
	if (await fsStat(ctx, cwd, "openspec/project.md", signal)) return {
		kind: "error",
		text: t("openspec/ 已存在（project.md 已初始化）", "openspec/ already exists (project.md present)")
	};
	await fsWrite(ctx, cwd, "openspec/project.md", [
		"# Project Context",
		"",
		"## Purpose",
	...
	return {
		kind: "success",
		text: t("已初始化 openspec/（project.md + specs/ + changes/）。下一步：/spec new <功能目标>", "Initialized openspec/ (project.md + specs/ + changes/). Next: /spec new <goal>")
	};
}
async function cmdNew(ctx, agent, cwd, goal, signal, t) {
	if (!goal.trim()) return {
		kind: "error",
		text: t("用法: /spec new <一句话目标>", "usage: /spec new <one-line goal>")
	};
	const zh = hasZh(goal);
	let answersText = "";
	const userQuestions = ctx.get("userQuestions");
	if (userQuestions !== void 0) try {
		...
		for (const item of Array.isArray(answer.answers) ? answer.answers : []) {
			...
			const label = item.id === "scope" ? zh ? "范围" : "Scope" : item.id === "constraints" ? zh ? "约束" : "Constraints" : zh ? "验收" : "Acceptance";
	...
	return {
		kind: "success",
		text: t("提案任务已交给 agent（生成 proposal.md / tasks.md / 规格增量，完成后自动校验）。面板将显示进度。", "Proposal task handed to the agent (proposal.md / tasks.md / spec deltas; auto-validated on completion). Watch the dock card for progress.")
	};
}
async function cmdList(ctx, cwd, signal, t) {
	const active = (await fsList(ctx, cwd, "openspec/changes", signal)).filter((e) => e.type === "directory" && e.name !== "archive").map((e) => e.name).sort();
	const caps = (await fsList(ctx, cwd, "openspec/specs", signal)).filter((e) => e.type === "directory" && !e.name.startsWith(".")).map((e) => e.name).sort();
	const lines = [];
	if (active.length === 0) lines.push(t("（无活跃变更 — /spec new <目标>）", "(no active changes — /spec new <goal>)"));
	...
	if (caps.length === 0) lines.push(t("（无能力规格 — specs/ 为空）", "(no capability specs — specs/ is empty)"));
	else lines.push(...caps.map((cap) => `- specs/${cap}/spec.md`));
	return {
		kind: "success",
		text: lines.join("\n")
	};
...
async function cmdShow(ctx, cwd, id, signal, t) {
	if (!id) return {
		kind: "error",
		text: t("用法: /spec show <id>", "usage: /spec show <id>")
	};
	const proposal = await fsRead(ctx, cwd, `openspec/changes/${id}/proposal.md`, signal);
	if (proposal === null) return {
		kind: "error",
		text: t(`未找到变更 openspec/changes/${id}/`, `no change at openspec/changes/${id}/`)
	};
...
/** The command a change card suggests next, per stage. */
const NEXT_COMMAND = {
	proposed: "approve",
	approved: "implement",
	implemented: "verify",
...
/**
* Read-only card: the current change from the specLoop projection plus x/y
* progress from the todos projection. Never touches the filesystem or the
* projection state.
*/
async function cmdStatus(ctx, agent, cwd, signal, t, lang) {
	const snapshot = ctx.sessionProjections.snapshot(agent.session);
	const specState = snapshot.values && snapshot.values.specLoop;
	const initialized = specState ? specState.initialized : false;
	const change = specState ? specState.change : null;
	if (!change) {
		if (!initialized) return {
			kind: "success",
			text: t("📐 openspec/ 未初始化 — 先运行 /spec init", "📐 openspec/ not initialized — run /spec init first")
		};
		return {
			kind: "success",
			text: t("📐 无活跃变更 — /spec new <目标> 创建一个", "📐 no active change — /spec new <goal> to create one")
		};
	}
	const todos = snapshot.values && snapshot.values.todos;
	const list = Array.isArray(todos) ? todos : null;
	const done = list ? list.filter((item) => item && item.status === "completed").length : null;
	const total = list ? list.length : null;
	const progress = total !== null ? ` ${done}/${total}` : "";
	const statusLabel = (STATUS_LABELS[lang] || STATUS_LABELS.en)[change.status] || change.status;
	const next = NEXT_COMMAND[change.status];
	return {
		kind: "success",
		text: [
			`📐 ${change.id} · ${statusLabel}${progress}`,
			change.title && change.title !== "(untitled)" && change.title !== change.id ? change.title : null,
			next ? t(`下一步：/spec ${next} ${change.id}`, `Next: /spec ${next} ${change.id}`) : null
...
async function cmdApprove(ctx, agent, cwd, id, signal, t) {
	if (!id) return {
		kind: "error",
		text: t("用法: /spec approve <id>", "usage: /spec approve <id>")
	};
	...
	if (current && current.id === id && current.status === "approved") return {
		kind: "success",
		text: t(`${id} 已是 approved`, `${id} is already approved`)
	};
	return {
		kind: "success",
...
async function cmdImplement(ctx, agent, cwd, id, signal, t) {
	if (!id) return {
		kind: "error",
		text: t("用法: /spec implement <id>", "usage: /spec implement <id>")
	};
	...
	if (!change || change.id !== id || !IMPLEMENTABLE.includes(change.status)) return {
		kind: "error",
		text: t(`${id} 尚未批准（状态: ${change && change.id === id ? change.status : "unknown"}）。先 /spec approve ${id}`, `${id} is not approved (status: ${change && change.id === id ? change.status : "unknown"}). Run /spec approve ${id} first`)
	};
	if (await fsRead(ctx, cwd, `openspec/changes/${id}/tasks.md`, signal) === null) return {
		kind: "error",
		text: t(`${id} 缺少 tasks.md`, `${id} has no tasks.md`)
	};
	agent.steer(createUserMessage({
		content: [{
			type: "text",
			text: implementPrompt(cwd, id)
		}],
		source: {
			kind: "plugin",
			plugin: ID
	...
	return {
		kind: "success",
		text: t(`实现任务已交给 agent（按 tasks.md 逐项实现并勾选，todo 面板跟踪进度）。`, `Implementation handed to the agent (tasks.md order, checkboxes ticked per task, todo panel tracks progress).`)
	};
}
async function cmdVerify(ctx, agent, cwd, arg, signal, t) {
	...
	if (!id) return {
		kind: "error",
		text: t("用法: /spec verify <id> [--deep]", "usage: /spec verify <id> [--deep]")
	};
	const dirInfo = await fsStat(ctx, cwd, `openspec/changes/${id}`, signal);
	if (!dirInfo || dirInfo.type !== "directory") return {
		kind: "error",
	...
	if (deltaTexts.length === 0) return {
		kind: "error",
		text: t(`${id} 没有规格增量文件`, `${id} has no spec delta files`)
	};
	let bashText = "";
	...
	if (bashBlocks.length > 0) {
		...
		for (const block of bashBlocks) {
			...
			try {
				...
				const out = result.stdout && typeof result.stdout === "object" && typeof result.stdout.text === "string" ? result.stdout.text : "";
				if (out.trim()) line += "\n" + out.trim().split("\n").slice(-15).join("\n");
	...
	let model = null;
	if (deep) model = options && options.model;
	else {
	...
	if (!provider || !model) return {
		kind: "error",
		text: t("无法确定验收模型（缺少 provider/model 配置）", "cannot determine the verify model (missing provider/model config)")
	};
	const filesText = (await collectWorkspaceFiles(ctx, cwd, signal)).map((f) => `--- ${f.path}\n${f.text}`).join("\n\n");
	...
	try {
		...
		return {
			kind: "error",
			text: t(`验收模型调用失败：${String(error)}`, `verify model call failed: ${String(error)}`)
		};
	}
	...
	if (verdicts.length === 0) {
		...
		return {
			kind: "error",
			text: t("验收模型未产出任何 OK|FAIL 判定（原始输出已写入 verify.md）", "verify model produced no OK|FAIL verdicts (raw output written to verify.md)")
		};
	}
	...
	const verifyMd = [
		...
		...failed > 0 ? [
			"## Failures",
			"",
			...verdicts.filter((v) => !v.pass).map((v) => `- ❌ ${v.requirement}: ${v.scenario} — ${v.reason}`),
			""
		] : [],
		"## Raw judge output",
		"",
	...
	const summary = t(`验收完成：${passed}/${verdicts.length} 通过（${failed} 失败）。报告：openspec/changes/${id}/verify.md`, `Verification done: ${passed}/${verdicts.length} passed (${failed} failed). Report: openspec/changes/${id}/verify.md`);
	if (failed > 0) {
...
async function cmdArchive(ctx, cwd, id, signal, t) {
	if (!id) return {
		kind: "error",
		text: t("用法: /spec archive <id>", "usage: /spec archive <id>")
	};
	const dirInfo = await fsStat(ctx, cwd, `openspec/changes/${id}`, signal);
	if (!dirInfo || dirInfo.type !== "directory") return {
		kind: "error",
	...
	for (const delta of deltas) {
		...
		await fsWrite(ctx, cwd, targetRel, (existing !== null ? "" : `# ${delta.cap} Specification\n\n## Requirements\n`) + result.text, signal);
	...
	try {
		...
		return {
			kind: "error",
			text: t(`归档移动失败：${String(error)}`, `archive move failed: ${String(error)}`)
		};
	}
	if (result.exitCode !== 0 || result.sandbox && result.sandbox.denied) return {
		kind: "error",
...
async function cmdValidate(ctx, cwd, id, signal, t) {
	...
	const ids = id ? [id] : changes.filter((e) => e.type === "directory" && e.name !== "archive").map((e) => e.name).sort();
	if (ids.length === 0) return {
		kind: "success",
		text: t("没有活跃变更可校验", "no active changes to validate")
	};
	const lines = [];
	let allOk = true;
	for (const changeId of ids) {
		const report = await validateChange(ctx, cwd, changeId, signal);
		if (report.ok) lines.push(`✅ ${changeId}`);
		else {
			allOk = false;
			lines.push(`❌ ${changeId}`);
...
async function cmdEdit(ctx, agent, cwd, id, signal, t) {
	if (!id) return {
		kind: "error",
		text: t("用法: /spec edit <id>", "usage: /spec edit <id>")
	};
	const dirInfo = await fsStat(ctx, cwd, `openspec/changes/${id}`, signal);
	if (!dirInfo || dirInfo.type !== "directory") return {
		kind: "error",
...
function apply(ctx) {
	...
	registerCommand(ctx);
	const autoValidateCounts = /* @__PURE__ */ new Map();
	ctx.on("session/event", (session, event) => {
		try {
			if (event.type !== "assistant/message") return;
			const m = messageText(event.data && event.data.message).match(CHANGE_ID_MARKER_RE);
			if (!m) return;
			const id = m[1];
			...
			Promise.resolve().then(async () => {
				...
				agent.steer(createUserMessage({
					content: [{
						type: "text",
						text: fixPrompt(cwd, id, report.errors.map((e) => "- " + e).join("\n"))
					}],
					source: {
						kind: "plugin",
						plugin: ID
...
//#endregion
export { STATUSES, apply, applySpecEvent, countTasks, hasZh, initSpecState, inject, mergeDelta, messageText, parseDelta, parseSpecArgs, projectionSchema, validateChange, validateDeltaText, viewSpecState };
